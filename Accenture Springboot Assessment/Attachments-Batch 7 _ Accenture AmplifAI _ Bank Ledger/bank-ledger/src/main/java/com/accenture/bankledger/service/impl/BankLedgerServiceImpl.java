...
public class BankLedgerServiceImpl implements BankLedgerService {
    ...
    private static final BigDecimal MIN_DEPOSIT_AMOUNT =
            new BigDecimal("1000.00");
    private static final BigDecimal MAX_DEPOSIT_AMOUNT =
            new BigDecimal("100000.00");
    private static final int BAD_REQUEST_STATUS = 400;
    private static final String DEPOSIT_AMOUNT_KEY = "depositAmount";
    ...
    private void validateRequest(LedgerOpenAccountRequest request) {
        ...
        if (request == null) {
            throw new BadRequestException(BAD_REQUEST_STATUS,
        ...
        if (request.getTermDepositDetails() == null) {
            errors.put("termDepositDetails",
                    "Term deposit details are required.");
        } else {
            BigDecimal depositAmount =
                    request.getTermDepositDetails().getDepositAmount();
            if (depositAmount == null) {
                errors.put(DEPOSIT_AMOUNT_KEY,
            ...
            if (request.getTermDepositDetails().getEffectiveDate() == null ||
                    request.getTermDepositDetails()
                            .getEffectiveDate().isBlank()) {
        ...
        if (request.getTermDepositMaturityDetails() == null) {
            errors.put("termDepositMaturityDetails",
                    "Maturity details are required.");
        } else {
            if (request.getTermDepositMaturityDetails().getAccountNumber()
                    == null
                    || request.getTermDepositMaturityDetails()
                            .getAccountNumber().isBlank()) {
                errors.put("accountNumber", "Account number is required.");
            }
            if (request.getTermDepositMaturityDetails().getAccountName()
                    == null
                    || request.getTermDepositMaturityDetails()
                            .getAccountName().isBlank()) {
    ...
    private Account generateAccount(LedgerOpenAccountRequest request) {
        return new Account(
                ...
                convertToLocalDate(
                        request.getTermDepositDetails().getEffectiveDate()),
                convertToLocalDate(
                        request.getTermDepositDetails().getExpiryDate())
        );
    }
